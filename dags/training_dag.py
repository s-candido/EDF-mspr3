...
def train_and_log_model(**context):
    ...
    print(f"Loaded {len(df)} records from {SOURCE_TABLE} for years {selected_years}")
    ...
    with mlflow.start_run(run_name=f"training_{best_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
        ...
        mlflow.log_param("training_years", str(selected_years))
...
with DAG(
    ...
    catchup=False,
    description="DAG for training ML models on selected years from aggregated_conso_weather table",
    tags=["training", "mlflow", "postgresql"],
    params={"selected_years": [2020, 2021, 2022]}
